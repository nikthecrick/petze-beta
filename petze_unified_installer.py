#                    (\".deadbug-mcp.py\", \"CRITICAL: Deadbugz malware artifact detected in tool call — known MCP supply chain attack.\"),\\n                    (\"productivity-suite-mcp.onrender.com\", \"CRITICAL: Known malicious MCP server detected — Deadbugz campaign IOC (CVE-2026).\"),\\n                    (\"deadbug-mcp\", \"CRITICAL: Deadbugz MCP malware identifier detected in tool call.\"),\\n                    (\".config/.cache/.sys/\", \"CRITICAL: Deadbugz hidden artifact path detected — known MCP supply chain IOC.\"),\\n!/usr/bin/env python3
import os
import json
import stat
import subprocess
...
# Package inspection module (base64 encoded for clean embedding)
_PKG_MODULE_B64 = ''
...
_KCP_MODULE_B64 = ''
...
with open(os.path.join(petze_dir, "bypass_secret.txt"), "w") as f:
    f.write("BYPASS")
...
if '_is_diluted' not in _proxy_src:
    ...
    _dil_new = (
        '                    _curr_intent = get_current_intent()\n'
        '                    _dilution_warning = ""\n'
        '                    _is_diluted = False\n'
        '                    if _curr_intent and new_intent:\n'
        '                        _curr_words = len(_curr_intent.split())\n'
        '                        _new_words = len(new_intent.split())\n'
        '                        if len(new_intent) < len(_curr_intent) * 0.7 or _new_words < _curr_words * 0.7:\n'
        '                            _is_diluted = True\n'
        '                            _dilution_warning = (\n'
        '                                "\u26a0\ufe0f WARNING: Proposed intent is LESS SPECIFIC.\\n"\n'
        '                                "A less specific intent gives the agent broader permissions.\\n\\n"\n'
        '                            )\n'
        '                    is_approved = False\n'
        '                    try:\n'
        '                        if sys.platform == "darwin":'
    )
    if _dil_old in _proxy_src:
        _proxy_src = _proxy_src.replace(_dil_old, _dil_new, 1)
        open(proxy_path, 'w').write(_proxy_src)
        print(f"{GREEN}\u2714 Intent dilution warning added{RESET}")
    else:
        print(f"{YELLOW}\u26a0 Intent dilution anchor not found{RESET}")

# ── Patch response guard with warn_only and grep/glob scanning ─────────────────
_resp_path = os.path.join(petze_dir, 'petze_response_guard.py')
if os.path.exists(_resp_path):
    _resp_src = open(_resp_path).read()
    if '_WARN_ONLY_PATTERNS' not in _resp_src:
        _old_scan = 'def scan_response('
        _new_scan = (
            '_WARN_ONLY_PATTERNS = [\n'
            '    "os.environ", "urllib.request", "predeploy_artifact",\n'
            '    "/tmp/cortex_", "run_predeploy", "settings.local.json",\n'
            '    ".claude/settings",\n'
            ']\n\n'
            'def scan_response('
        )
        if _old_scan in _resp_src:
            _resp_src = _resp_src.replace(_old_scan, _new_scan, 1)
            print(f"{GREEN}\u2714 WARN_ONLY_PATTERNS added to response guard{RESET}")
    if 'grep/glob' not in _resp_src:
        _old_empty = '        if not full_text:\n            return False, ""'
        _new_empty = (
            '        # Scan raw result for grep/glob responses\n'
            '        if not full_text:\n'
            '            if isinstance(result, str): full_text = result\n'
            '            elif isinstance(result, dict): full_text = str(result)\n'
            '            elif isinstance(result, list): full_text = " ".join(str(x) for x in result)\n'
        )
        if _old_empty in _resp_src:
            _resp_src = _resp_src.replace(_old_empty, _new_empty, 1)
            _resp_src += '  # grep/glob responses scanned\n'
            print(f"{GREEN}\u2714 Grep/glob scanning added to response guard{RESET}")
    open(_resp_path, 'w').write(_resp_src)
...
if __name__ == "__main__":
    port = 8443
    print(f"\033[34m(‾_‾)\033[0m  Starting Petze SOC on http://localhost:{port}")
...
def init_colors():
    ...
    curses.init_pair(C_DIM,      curses.COLOR_WHITE,  -1)  # readable grey
...
def draw(stdscr, logs, log_lines, prov, stats, tick):
    ...
    # ── Header bar ────────────────────────────────────────────────────────────
    header = f" (‾_‾)  PETZE GUARD  │  Terminal Dashboard  │  {now_str}  │  Q quit  R refresh "
...
echo -e "\\033[34m(‾_‾)\\033[0m  \\033[92mPETZE GUARD IS ACTIVE. Zero-trust sandbox engaged.\\033[0m"
"""
...
shell_injection += '\npetze-setmodel() {\n'
shell_injection += '    if [ -z "$1" ]; then\n'
shell_injection += '        local _cur=$(python3 -c "import json,os; d=json.load(open(os.path.expanduser(os.path.join(chr(126),chr(46)+chr(112)+chr(101)+chr(116)+chr(122)+chr(101),chr(99)+chr(111)+chr(110)+chr(102)+chr(105)+chr(103)+chr(46)+chr(106)+chr(115)+chr(111)+chr(110))))); print(d.get(chr(111)+chr(112)+chr(101)+chr(110)+chr(99)+chr(111)+chr(100)+chr(101)+chr(95)+chr(109)+chr(111)+chr(100)+chr(101)+chr(108)),end=chr(0))" 2>/dev/null | tr -d "\\0")\n'
shell_injection += '        echo -e "\\n\\033[34m(\u203e_\u203e)\\033[0m  Current model: \\033[97m$_cur\\033[0m"\n'
shell_injection += '        echo -e "\\033[90mUsage: petze-setmodel <model> (e.g. hy3-free, big-pickle)\\033[0m\\n"\n'
shell_injection += '        return\n'
shell_injection += '    fi\n'
shell_injection += f'    python3 {os.path.join(petze_dir, "petze_setmodel.py")} "$1"\n'
shell_injection += '    echo -e "\\033[92m\u2714 OpenCode model set to: opencode/$1\\033[0m"\n'
shell_injection += '    echo -e "\\033[90mStart a new session to apply.\\033[0m"\n'
shell_injection += '}\n'


shell_injection += '\npetze-kcp() {\n'
shell_injection += '    # See petze-kcp command — managed via petze_kcp.py\n'
shell_injection += '    local KCP_JSON="$HOME/.petze/petze_kcp.json"\n'
shell_injection += '    local KCP_LOG="$HOME/.petze/kcp_session.json"\n'
shell_injection += '    if [ -z "$1" ]; then\n'
shell_injection += '        echo -e "\\n\\033[34m(\u203e_\u203e)\\033[0m  Petze Kill Chain Patterns\\n"\n'
shell_injection += '        echo -e "  \\033[97mpetze-kcp list\\033[0m                    Show all patterns"\n'
shell_injection += '        echo -e "  \\033[97mpetze-kcp status\\033[0m                  Last session detections"\n'
shell_injection += '        echo -e "  \\033[97mpetze-kcp threshold <ID> <n>\\033[0m   Set detection threshold"\n'
shell_injection += '        echo -e "  \\033[97mpetze-kcp disable <ID>\\033[0m           Disable a pattern"\n'
shell_injection += '        echo -e "  \\033[97mpetze-kcp enable <ID>\\033[0m            Re-enable a pattern"\n'
shell_injection += '        echo -e "  \\033[97mpetze-kcp reset\\033[0m                   Restore default thresholds"\n'
shell_injection += '        echo -e "  \\033[97mpetze-kcp edit\\033[0m                    Open pattern library in editor\\n"\n'
shell_injection += '        return 0\n'
shell_injection += '    fi\n'
shell_injection += '    python3 "$HOME/.petze/petze_kcp.py" "--$1" "${@:2}" 2>/dev/null || echo -e "  \\033[91m\u2716 Unknown command: $1\\033[0m"\n'
shell_injection += '}\n'

shell_injection += '\npetze-update() {\n'
shell_injection += '    local MODULE="${1:-}"\n'
shell_injection += '    if [ -z "$MODULE" ]; then\n'
shell_injection += '        local _cache="$HOME/.petze/package_cache.json"\n'
shell_injection += '        local _last="never synced"\n'
shell_injection += '        echo -e "\\n\\033[34m(‾_‾)\\033[0m  Petze Update Manager\\n"\n'
shell_injection += '        echo -e "  \\033[97mosv\\033[0m      Package threat intelligence"\n'
shell_injection += '        echo -e "  \\033[90mpetze\\033[0m    Petze Guard core               Update server: \\033[90mcoming soon\\033[0m"\n'
shell_injection += '        echo -e "\\n\\033[90mRun: petze-update <module>\\033[0m\\n"\n'
shell_injection += '        return\n'
shell_injection += '    fi\n'
shell_injection += '    if [ "$MODULE" = "osv" ]; then\n'
shell_injection += '        echo -e "\\033[34m(‾_‾)\\033[0m  Refreshing package threat intelligence..."\n'
shell_injection += f'        python3 {os.path.join(petze_dir, "petze_package_guard.py")} --update-cache\\n'
shell_injection += '    elif [ "$MODULE" = "kcp" ]; then\\n'
shell_injection += '        echo -e "\\\\033[34m(\u203e_\u203e)\\\\033[0m  Kill Chain Pattern Library"\\n'
shell_injection += f'        python3 {os.path.join(petze_dir, "petze_kcp.py")} --list\\\\n'
shell_injection += '    else\n'
shell_injection += '        echo -e "\\033[91m✖ Unknown module: $MODULE\\033[0m"\n'
shell_injection += '        return 1\n'
shell_injection += '    fi\n'
shell_injection += '}\n'
...
petze-elevate() {
    ...
    echo -n "Type 'ROOT' to confirm: "; read confirm
...
" "$_brief_path" 2>/dev/null)
            ...
            echo "$PETZE_INTENT" > ~/.petze/intent.txt
                ...
                echo -e "\n  $_structured\n"
                echo -n "Confirm? (Enter to accept, or type a correction): "; read _correction
    ...
    if [ -z "$OC_BIN" ] || [ ! -x "$OC_BIN" ]; then
        for _p in "$HOME/.opencode/bin/opencode" "$HOME/.local/bin/opencode" \
                  "$HOME/.bun/bin/opencode" "/usr/local/bin/opencode" \
                  "/opt/homebrew/bin/opencode"; do
            [ -x "$_p" ] && { OC_BIN="$_p"; break; }
        done
    fi
    if [ -z "$OC_BIN" ]; then
        echo -e "\033[91m(‾_‾) OpenCode not found.\033[0m"; return 1
    fi
    "$OC_BIN" "$@"
    sleep 0.5
    clear
    ...
    # Find the most recent session ID from the agent
        _show_session_id() {
    # Try SQLite first (OpenCode v2+), fall back to JSON files (older versions)
    local _OC_SID=""
    local _db=~/.local/share/opencode/opencode.db
    if [ -f "$_db" ]; then
        _OC_SID=$(sqlite3 "$_db" "SELECT id FROM session ORDER BY time_updated DESC LIMIT 1;" 2>/dev/null)
    fi
    if [ -z "$_OC_SID" ]; then
        _OC_SID=$(find ~/.local/share/opencode/storage/session_diff -name 'ses_*.json' \
            2>/dev/null -exec ls -t {} + 2>/dev/null | head -1 | sed 's|.*/||;s|\.json||' 2>/dev/null)
    fi
    if [ "$PETZE_AGENT" = "Claude Code" ]; then
        _OC_SID=$(find ~/.claude/projects -name "*.jsonl" 2>/dev/null -exec ls -t {} + 2>/dev/null | head -1 | sed 's|.*/||;s|\.jsonl||' 2>/dev/null)
    fi
    local _B="\033[34m"
    local _LB="\033[94m"
    local _W="\033[97m"
    ...
    local _R="\033[0m"
    local _resume_cmd=""
    [ -n "$_OC_SID" ] && _resume_cmd="opencode -s ${_OC_SID}"
    [ "$PETZE_AGENT" = "Claude Code" ] && [ -n "$_OC_SID" ] && _resume_cmd="claude --resume ${_OC_SID}"
...
" "$_brief_path" 2>/dev/null)
    ...
    if [ -n "$_OC_SID" ]; then
        ...
        echo -e "${_B} █▓▓▓▓▓▓▓▓▓▓▓▓▓▓█ ${_R}"
        echo -e "${_B} █▓▓${_W}━━${_B}▓▓▓▓${_W}━━${_B}▓▓▓▓█ ${_R}  ${_DIM}Session ID not found${_R}"
        echo -e "${_B} █▓▓▓▓▓▓▓▓▓▓▓▓▓▓█ ${_R}  ${_DIM}Scroll up to see Claude Code${_R}"
        echo -e "${_B} █▓▓▓▓${_W}████${_B}▓▓▓▓▓▓█ ${_R}  ${_DIM}resume command above${_R}"
        echo -e "${_B}  █▓▓▓▓▓▓▓▓▓▓▓▓█  ${_R}"
    fi
    echo -e "${_B}   ▀█▓╱╲▓▓╱╲▓█▀   ${_R}  ${_DIM}firewall active · petze.xyz${_R}"
    echo
