...
" "$_brief_path" 2>/dev/null)
    ...
    # Find the most recent session ID from the agent
        _show_session_id() {
    local _OC_SID=$(find ~/.local/share/opencode/storage/session_diff -name 'ses_*.json' 2>/dev/null -exec ls -t {} + 2>/dev/null | head -1 | sed 's|.*/||;s|\.json||' 2>/dev/null)
...
print(f"""
{B}    ▄▄▄▄▄▄▄▄▄▄    {X}  {G}INSTALLATION COMPLETE{X}
{B}  ▄█▓▓▓▓▓▓▓▓▓▓█▄  {X}  {D}──────────────────────────────{X}
{B} █▓▓▓▓▓▓▓▓▓▓▓▓▓▓█ {X}  {D}Activate your shell functions:{X}
{B} █▓▓{W}━━{B}▓▓▓▓{W}━━{B}▓▓▓▓█ {X}  {W}source ~/{rc_file}{X}
{B} █▓▓▓▓▓▓▓▓▓▓▓▓▓▓█ {X}
{B} █▓▓▓▓{W}████{B}▓▓▓▓▓▓█ {X}  {D}Then type{X} {W}petze-help{X}
{B}  █▓▓▓▓▓▓▓▓▓▓▓▓█  {X}
{B}   ▀█▓╱╲▓▓╱╲▓█▀   {X}  {D}petze.xyz{X}
""")
