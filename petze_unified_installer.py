...
" "$_brief_path" 2>/dev/null)
    ...
    # Find the most recent session ID from the agent
        _show_session_id() {
    local _OC_SID=$(ls -t ~/.local/share/opencode/storage/session_diff/ses_*.json 2>/dev/null | head -1 | sed 's|.*/||;s|\.json||' 2>/dev/null)
